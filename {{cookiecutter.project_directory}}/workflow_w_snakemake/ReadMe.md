...
After creating the virtual environment using the provided `requirements.txt`, you can open a Powershell or command prompt on Windows or terminal, activate this virtual environment, e.g., by doing `.\.venv\Scripts\activate.ps1` on Windows Powershell, and do `snakemake --version` which should print out a number or `snakemake --help` which should print out a long list of arguments and commands that gives you a lot of freedom to design your workflow and manage it.
